...
namespace WebAPIGroup2.Respository.Implement
{
    public class TemplateRepo : GenericRepository<Template>, ITemplateRepo
    {
        ...
        {
            return await _context.Templates.Include(i => i.TemplateImages).Include(d => d.DescriptionTemplates).Include(c=>c.CategoryTemplates).Include(s=>s.TemplateSizes).Include(r=>r.Reviews).ThenInclude(u=>u.User).FirstOrDefaultAsync(i => i.Id == id);
        }
        ...
        {
            var list = _context.Templates.Include(i => i.TemplateImages).Include(d => d.DescriptionTemplates).Include(c => c.CategoryTemplates).Include(s => s.TemplateSizes).Include(r => r.Reviews).ThenInclude(u => u.User).AsQueryable();
